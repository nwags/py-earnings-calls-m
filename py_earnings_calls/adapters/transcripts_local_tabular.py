...
_CANDIDATE_PROVIDER_COLUMNS = ["provider", "source_provider", "transcript_provider"]
_CANDIDATE_PROVIDER_CALL_ID_COLUMNS = ["provider_call_id", "call_id", "id", "transcript_id"]
...
class LocalTabularTranscriptAdapter(TranscriptBulkAdapter):
    provider = "local_tabular"
    ...
    def load_documents(self, dataset_path: str) -> list[TranscriptDocument]:
        ...
        df = read_tabular_dataframe(path, allow_parquet=True)

        provider_col = first_present_column(df.columns, _CANDIDATE_PROVIDER_COLUMNS)
        ...
        for row in df.to_dict(orient="records"):
            provider = normalize_text(row.get(provider_col)) if provider_col else None
